...
When you register routes with controllers, middleware can be declared directly on
the controller class or on individual controller methods with the `@middleware`
decorator. This keeps a controller's middleware next to the code it protects
instead of repeating it at every route registration.
...
Class-level decorators apply to every action of the controller, including routes
generated by `Router.apiResource()`. Method-level decorators apply only to that
action:
...
Router.post('/login', [LoginController, 'create']); // runs GuestMiddleware
Router.delete('/login', [LoginController, 'destroy']); // no middleware
```

The decorator accepts every shape a route middleware does — callbacks, middleware
classes, and instances exposing a `handle` method — either variadically or as an
array:

```ts
@middleware(AuthMiddleware, new AuditMiddleware('billing'))
...
Decorator middleware run after global and group middleware but before any
middleware passed at the route registration site. For a controller route the full
order is:
...
::: tip
`@middleware` uses the same metadata mechanism as `@Bind`, so it works with both
TypeScript's `experimentalDecorators` and the TC39 standard decorators.
:::
...
```ts
await Router.group('/admin', 'routes/admin').middleware([
  AuthMiddleware,
  AdminMiddleware,
]);
```
...
```txt
Global middleware -> Group middleware -> @middleware (class) -> @middleware (method) -> Route middleware -> Route handler
```
