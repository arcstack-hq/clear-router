...
H3 handlers can return response values directly. You can still return H3-native responses when you need explicit control.
...
Direct `POST` returns default to `201`; other methods default to `200`. HTML-like strings are sent as `text/html`, API/XHR string responses as `text/plain`, objects/arrays as JSON, and H3 `HTTPResponse` values keep their own status and headers.
